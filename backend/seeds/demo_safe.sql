...
ON CONFLICT (id) DO UPDATE SET email=EXCLUDED.email, name=EXCLUDED.name, status='active', password_hash=EXCLUDED.password_hash, updated_at=now();
...
ON CONFLICT (id) DO UPDATE SET role='primary', status='active', updated_at=now();
...
ON CONFLICT (id) DO UPDATE SET title=EXCLUDED.title, status='published', updated_at=now();
...
ON CONFLICT (id) DO UPDATE SET prompt=EXCLUDED.prompt, points=EXCLUDED.points, options=EXCLUDED.options, rubric=EXCLUDED.rubric, answer_key=EXCLUDED.answer_key, updated_at=now();
